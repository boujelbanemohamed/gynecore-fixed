        ...
        <div style={{ display: 'grid', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingBottom: 20, borderBottom: '1px solid #eee' }}>
            ...
            <div>
              ...
              <div style={{ color: '#666', fontSize: 13 }}>Secretaire</div>
            </div>
          </div>
          ...
          <div>
            ...
            <input type="tel" name="phone" value={form.phone} onChange={handleChange} disabled={!editing} placeholder="+216 XX XXX XXX" style={{ width: '100%', padding: '10px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, backgroundColor: editing ? '#fff' : '#f9f9f9', color: '#333', maxWidth: 600 }} />
          </div>
          <div style={{ padding: '12px 16px', backgroundColor: '#f0f7f5', borderRadius: 8, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <span style={{ fontSize: 12, color: '#888' }}>Role</span>
              <div style={{ fontWeight: 600, fontSize: 14, color: '#1a5c4a' }}>Secretaire</div>
            </div>
            <div>
              <span style={{ fontSize: 12, color: '#888' }}>Medecin</span>
              <div style={{ fontWeight: 600, fontSize: 14, color: '#1a5c4a' }}>{(user as any)?.doctorId ? 'Associe a un medecin' : 'Non associe'}</div>
            </div>
          </div>
          {editing && (
            <div style={{ display: 'flex', gap: 12, paddingTop: 12, borderTop: '1px solid #eee' }}>
              ...
              <button className="btn" onClick={() => { setEditing(false); setForm({ firstName: user?.firstName || '', lastName: user?.lastName || '', email: user?.email || '', phone: (user as any)?.phone || '' }); }} style={{ backgroundColor: '#f0f0f0' }}>Annuler</button>
            </div>
          )}
        </div>
      </div>
