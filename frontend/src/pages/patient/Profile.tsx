...
const PatientProfile: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      // TODO: patient profile update endpoint
      const res = await fetch('/api/patient/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('token') }, body: JSON.stringify(form) });
      ...
      <div className="card" style={{ maxWidth: 600 }}>
        <div style={{ display: 'grid', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingBottom: 20, borderBottom: '1px solid #eee' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', backgroundColor: '#2196F3', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 24, fontWeight: 700 }}>
              {(user?.firstName?.[0] || '').toUpperCase()}{(user?.lastName?.[0] || '').toUpperCase()}
            </div>
            <div>
              <div style={{ fontWeight: 600, fontSize: 18 }}>{user?.firstName} {user?.lastName}</div>
              <div style={{ color: '#666', fontSize: 13 }}>Patiente</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#555' }}>Adresse</label>
              <input type="text" name="address" value={form.address} onChange={handleChange} disabled={!editing} style={{ width: '100%', padding: '10px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, backgroundColor: editing ? '#fff' : '#f9f9f9', color: '#333' }} />
            </div>
          </div>
          {editing && (
            <div style={{ display: 'flex', gap: 12, paddingTop: 12, borderTop: '1px solid #eee' }}>
              ...
              <button className="btn" onClick={() => { setEditing(false); setForm({ firstName: user?.firstName || '', lastName: user?.lastName || '', email: user?.email || '', phone: (user as any).phone || '', address: (user as any).address || '' }); }} style={{ backgroundColor: '#f0f0f0' }}>Annuler</button>
            </div>
          )}
        </div>
      </div>
